...
export default function Header() {
  ...
  return (
    <>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      Servicios
                      <ChevronDown
                        className={`ml-1 h-4 w-4 transform transition-transform ${isServicesMenuOpen ? 'rotate-180' : ''} ${txtClass}`}
                      />
                      <span
                        className={`absolute bottom-0 left-0 w-full h-[2px] bg-primary origin-center transition-transform ${isServicesMenuOpen ? 'scale-x-100' : 'scale-x-0'}`}
                      />
                    </button>
                    {isServicesMenuOpen && (
                      <div
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className={`group flex items-center pb-1 hover:text-primary ${txtClass}`}
                    >
                      <Briefcase className={`mr-2 h-4 w-4 ${txtClass}`} /> Portafolio
                      <ChevronDown
                        className={`ml-1 h-4 w-4 transform transition-transform ${isPortfolioOpen ? 'rotate-180' : ''} ${txtClass}`}
                      />
                      <span
                        className={`absolute bottom-0 left-0 w-full h-[2px] bg-primary origin-center transition-transform ${isPortfolioOpen ? 'scale-x-100' : 'scale-x-0'}`}
                      />
                    </button>
                    {isPortfolioOpen && (
                      <div
      ...
      {/* Mobile sidebar */}
      <div
        className={`fixed top-0 left-0 bottom-0 z-50 w-full max-w-[90vw] sm:w-64 bg-sidebar text-[rgb(var(--color-sidebar-foreground))] dark:text-white transform transition-transform md:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
              ...
              return (
                <div key={link.id}>
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`h-5 w-5 transform transition-transform ${isMobileServicesSubMenuOpen ? 'rotate-180' : ''} ${txtClass}`}
                    />
                  </button>
